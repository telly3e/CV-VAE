...
def encode_videos(vae_path, video_dir, latent_dir, height=576, width=1024):
    ...
    """
    vae3d = CVVAESD3Model.from_pretrained(vae_path, subfolder="vae3d_sd3", torch_dtype=torch.float16)
...
def decode_latents(vae_path, latent_dir, video_dir, height=576, width=1024):
    ...
    """
    vae3d = CVVAESD3Model.from_pretrained(vae_path, subfolder="vae3d", torch_dtype=torch.float16)
    vae3d.requires_grad_(False).cuda()
